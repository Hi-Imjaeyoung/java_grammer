...
//        String과 int의 형변환
        ...
        stA = String.valueOf(a1);
        int c1 = Integer.parseInt(stA);
...
//        문자열의 길이는 length 공백도 포함
        String myst = "hello";
        System.out.println(myst.length());
//        indexOf
        String myst6 = "hello java java";
        System.out.println(myst6.indexOf("j")); // 특정 문자열의 위치를 반환 (가장 먼저 나오는 문자열의 위치를 리턴) 공백도 문자열
...
//        replaceAll : 정규 표현식을 사용 가능
        String replace_test2 = "hello world";
        System.out.println(replace_test2.replaceAll("world","java"));
        System.out.println(replace_test2.replace("world","hoon"));
...
//        isEmpty : null과는 다름에 유의 "" 과 null은 다름
        ...
        System.out.println(empty_test==null); //false;
        empty_test = null;

        //false null이라서 연산 자체가 불가 nullPointException // runtime error
        //System.out.println(empty_test.isEmpty());

//        join : String[] -> String 합하면서 기준이 존재. ex) index마다 구분
        String[] join_test = {"oh","happy","friday","^0^","~~~"};
        System.out.println(String.join(" ",join_test));

//        StringBuffer
        StringBuffer sb_test = new StringBuffer("hello");
        System.out.println(sb_test.toString()); //StringBuffer 타입이 가짐 return시 유의하세요
        sb_test.append(" world");
        System.out.println(sb_test.toString());
        sb_test.insert(5," java"); // index 앞으로 추가된다.
        System.out.println(sb_test);
        sb_test.substring(0,5);
        System.out.println(sb_test); // substring은 값자체를 변화시키지는 않는다.
        sb_test.delete(5,10);
        System.out.println(sb_test);

//        StringBuilder 성능은 builder가 굿 , 스레드 safe x
        StringBuilder sbuild_test = new StringBuilder("hello");
        sbuild_test.append(" world");
        System.out.println(sbuild_test);
    }
}
